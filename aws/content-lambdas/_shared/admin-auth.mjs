...
export const jsonResponse = (
  ...
  statusCode,
  headers: {
    ...buildCorsHeaders(allowedMethods),
    ...headers,
  },
...
export const parseJsonBody = (event) => {
  ...
  try {
    ...
    throw new AdminAuthError(400, "Request body must contain valid JSON.");
  }
};
...
const getAdminConfig = () => {
  ...
  const ttlSeconds = Math.max(
    900,
    Number(process.env.ADMIN_TOKEN_TTL_SECONDS ?? 60 * 60 * 24 * 7)
  );
...
const issueAdminToken = (username, secret, ttlSeconds) => {
...
export const authenticateAdminCredentials = (rawUsername, rawPassword) => {
  const { username, password, secret, ttlSeconds } = getAdminConfig();
  ...
  if (
    !safeCompare(candidateUsername, username) ||
    !safeCompare(candidatePassword, password)
  ) {
    throw new AdminAuthError(401, "Invalid admin credentials.");
  }

  return issueAdminToken(username, secret, ttlSeconds);
};
